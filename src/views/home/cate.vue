<template>
    <div id="cate">
        <myHeader title="诗橙尝鲜" backUrl="index"></myHeader>
        <div class="content" v-for="items in supplierList" @click="toDetail(items.supplierId)">
            ...
            <p class="title">{{items.storeName}}<span class="am-fr"><i class="c-orange">{{items.spendPer}}</i>元/人</span></p>
        </div>
...
<script>
  ...
  export default {
    data() {
      return {
        supplierList: '',
        defaultImg: '../../assets/test-img-1.jpg',
        loadStatus: false,
        page: 1
      }
    },
    ...
    methods: {
      toDetail(id) {
        this.$router.push({ name: 'sellerDetail', query: { id: id }})
      },
      getFetch() {
        let self = this
        Indicator.open('加载中...');
        api.postData('/app/supplierList.htm',{recommend: 'YES',saleType: 'O2O',pageNumber: self.page}).then(function(data){
          Indicator.close()
          if (data.code == 1) {
            if(self.page>1){
