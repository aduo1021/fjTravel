<template>
    <div id="editeMobile">
      ...
      <div class="content">
        <mt-field label="手机号码" :readonly="true" :value="phone" type="tel"></mt-field>
        <mt-field label="短信验证码" v-model="code" class="bb-1px" placeholder="验证码" type="tel" :attr="{maxlength: 6,required: true}">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      submit() {
        Indicator.open('加载中...')
        api.postData('/app/appChangeMobile.htm', {code: this.code}).then((data) =>{
          Indicator.close()
          Toast(data.message)
          if (data.code == 1) {
            api.setSession('md5',data.md5UserBaseId)
            self.$router.push('editeMobile2')
          }
        })
      }
    }
